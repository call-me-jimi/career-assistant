...
- **Keep the `<<<cache-break>>>` line when versioning a looped prompt.** `generate_cover_letter*` and `simulate_hiring_manager` run up to three times per session over the same CV and job description. The marker line splits the user prompt: everything above it is sent to Anthropic as a cached block (`_user_content()` in `backend/llm/service.py`), everything below it is what changes between rounds (the draft, the last feedback). Anything that varies per call must sit below the marker, or every call misses the cache. Other providers get the marker stripped. `traces.cache_read_tokens` / `cache_write_tokens` show whether it hits.
...
- **`applied_at` is the user's answer, not a side effect of exporting.** The `log_application` node asks whether the application was actually submitted and for tracker notes, once per session. It sits *after* `export_node` — you need the cover letter in hand before you can say you sent it — and *before* `export_sheets_node`, because both answers also fill the spreadsheet row (`Notes`, and `Status`/`Submission`, which say `Draft` and stay blank until the user confirms). Declining the export (`none`) still reaches the questions. `no` leaves the journey a `draft`. The migration that dated pre-tracker journeys from their cover letter is therefore one-shot, guarded by `PRAGMA user_version` in `backend/storage/db.py`; rerunning it — as it did on every startup before v0.15.0 — would promote every deliberate "not sent yet" to `applied`.
...
- **Tag releases with semantic versioning.** Not every commit needs a tag — tag when a meaningful feature or fix is complete and merged to `main`. Use `vMAJOR.MINOR.PATCH`: bump `MINOR` for new features, `PATCH` for bug fixes, `MAJOR` for breaking changes. Current version: `v0.22.1`. After tagging, push with `git push origin <tag>`. At the end of any session that ships a feature or fix, remind the user to tag if appropriate.
